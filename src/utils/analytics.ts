import type { AnalyticsSnapshot, AppSettings, DateRange, TrainingLog, TrendPoint } from '@/types';
...
export const filterLogsByRange = (logs: TrainingLog[], range: DateRange) => {
  if (range.preset === 'all') return logs;
  if (range.preset === '7d') {
...
const byWeek = (logs: TrainingLog[]) => {
  const bucket = new Map<string, TrainingLog[]>();
  logs.forEach((log) => {
...
export const calculateWeeklyStreaks = (logs: TrainingLog[], weeklyGoal: number) => {
  ...
  for (const [, weekLogs] of reversed) {
    if (weekLogs.length >= weeklyGoal) currentStreak += 1;
    else break;
  }
...
const makeTrend = (logs: TrainingLog[], mapper: (items: TrainingLog[]) => TrendPoint) =>
  byWeek(logs).map(([, items]) => mapper(items));
